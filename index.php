...
require 'include/syssettings.php';
//
//CHECK INSTALL
//
...
//Global varibles to optimize for various website plugins
define("PHPROXY_ROOT", $_SERVER['DOCUMENT_ROOT']);
...
function write_to_log($url, $responsecode) {
    ...
    fwrite($file, $_SERVER['REMOTE_ADDR'].' '.date("h:i:s A").'  '.$url.' '.$responsecode."\n");
    fclose($file);
}
...
if ($_content_type == 'text/css') {
    ...
    for ($i = 0, $count_i = count($matches[0]); $i < $count_i; ++$i) {
        ...
        if (isset($tags[$tag])) {
			...
			//Fix for torrents and magnet links, PS: Seed you leechs
			$noproxy = array('magnet:');
			foreach ($noproxy as $x) {
			$pos = strpos($attrs['href'], $x);
			if ($pos !== 0) {
    ...
    if ($_flags['include_form'] && !isset($_GET['nf'])) {
        $_url_form =  '<div id="form" style="width:100%;margin:0;text-align:center;border-bottom:1px solid #725554;color:#000000;background-color:#F2FDF3;font-size:12px;font-weight:bold;font-family:Arial, Helvetica, sans-serif;padding:4px;">'
                        ...
                        . ' [go: <a href="' . $_script_url . '?' . $_config['url_var_name'] . '=' . encode_url($_url_parts['prev_dir']) .' ">Go Back</a>, <a href="' . $_script_base . '">main page</a>]'
                        . '<br /><hr />';

//Set Global Varibles for Includes
$script_base_phproxy = $_SERVER['SCRIPT_FILENAME'];
$script_base_phproxy = preg_replace('/index.php/s','', $script_base_phproxy);
define("script_base_phproxy", $script_base_phproxy);
...
HTML;
$_response_body = preg_replace('#\<\s*head(.*?)>#si', "<head>".$googleacode , $_response_body, 1);
	}}
    }
}
...
$_response_body_edit = "this plugin is not working please contact its deviliper";

$parseurl = parse_url($_url);

$plurl = "plugins/".$parseurl['host'].'.php';

if(file_exists($plurl)) {
  include $plurl;
  echo $_response_body_edit;
